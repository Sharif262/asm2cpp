/* std::string argparse::details::repr<std::vector<float, std::allocator<float> >
   >(std::vector<float, std::allocator<float> > const&) */

void argparse::details::repr<std::vector<float,std::allocator<float>>>(vector *param_1)
{
  ulong uVar1;
  char *pcVar2;
  float *pfVar3;
  ...
  if (1 < uVar7) {
    ...
    std::
    for_each_abi_ne200100_<std::__wrap_iter<float_const*>,argparse::details::repr<std::vector<float,std::allocator<float>>>(std::vector<float,std::allocator<float>>const&)::_lambda(auto:1_const&)_1_>
  ...
  if (pfVar4 != pfVar3) {
    repr<float>((float *)(*(long *)(param_1 + 8) + -4));
    ppcVar6 = (char **)local_180;
    if (-1 < (char)local_169) {
