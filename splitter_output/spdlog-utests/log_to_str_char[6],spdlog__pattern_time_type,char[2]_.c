...
/* std::string log_to_str<char [6], spdlog::pattern_time_type, char [2]>(std::string const&, char
   const (&) [6], spdlog::pattern_time_type const&, char const (&) [2]) */

void log_to_str<char[6],spdlog::pattern_time_type,char[2]>
               (string *param_1,char *param_2,pattern_time_type *param_3,char *param_4)
{
  ...
  *(undefined1 *)(plVar9 + 0xf) = 0;
  ...
  if (sVar11 < 0x17) {
    ...
    if (sVar11 == 0) goto LAB_100029e0c;
  }
  else {
    uVar2 = 0x19;
    if ((sVar11 | 7) != 0x17) {
