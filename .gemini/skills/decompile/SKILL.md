...
2. Gemini refines parsed C into clean C++
...
**In chat:** type `/decompile <path>` and send. Gemini runs the full pipeline and replies in chat.
...
**File location:** If the agent is sandboxed (can't leave the repo), the user must either put the file inside the repo or grant permission to access external paths. Otherwise the agent cannot read it.
